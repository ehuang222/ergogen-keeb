...
//    socket_hole_size: default is 1.7
//      allows to define the size of the socket through-hole pads
//    socket_hole_drill: default is 0.85
//      allows to define the drill size of the MCU through-holes (e.g. 0.8–0.9 mm for
...
//
// This footprint includes modifications by @Scybin:
// - change mcu through_holes to 0.85mm to support no-solder spring headers
...
module.exports = {
  params: {
    ...
    socket_hole_size: 1.7,
    socket_hole_drill: 0.85,
  ...
  (segment (start ${p.eaxy(3.099695, -0.725 + (row_num * 2.54) - 12.7)}) (end ${p.eaxy(2.594695, -0.22 + (row_num * 2.54) - 12.7)}) (width 0.25) (layer "B.Cu"))
    ...
    const gen_socket_row = (row_num, pin_name_left, pin_name_right, show_via_labels, show_silk_labels) => {
      ...
      let socket_row_base = `
    ${''/* Socket Holes */}
    (pad "${socket_hole_num_left}" thru_hole circle (at -7.62 ${-12.7 + row_offset_y} ${p.r}) (size ${p.socket_hole_size} ${p.socket_hole_size}) (drill ${p.socket_hole_drill}) (layers "*.Cu" "*.Mask") ${p.reversible && (row_num < 4 || !p.only_required_jumpers) ? p.local_net(socket_hole_num_left).str : net_left})
    (pad "${socket_hole_num_right}" thru_hole circle (at 7.62 ${-12.7 + row_offset_y} ${p.r}) (size ${p.socket_hole_size} ${p.socket_hole_size}) (drill ${p.socket_hole_drill}) (layers "*.Cu" "*.Mask") ${p.reversible && (row_num < 4 || !p.only_required_jumpers) ? p.local_net(socket_hole_num_right).str : net_right})
      `
      let socket_row_vias = `
    ${''/* Inside VIAS */}
    ...
    const common_top = `
  (footprint "scybin:mcu_nice_nano"
    (layer "${p.side}.Cu")
    ${p.at}
  ...
  ${''/* Controller outline */}
    ...
    const extra_pins = `
    (pad "25" thru_hole circle (at ${invert_pins ? '' : '-'}5.08 10.16 ${p.r}) (size ${p.socket_hole_size} ${p.socket_hole_size}) (drill ${p.socket_hole_drill}) (layers "*.Cu" "*.Mask") ${p.P101})
    (pad "26" thru_hole circle (at ${invert_pins ? '' : '-'}2.54 10.16 ${p.r}) (size ${p.socket_hole_size} ${p.socket_hole_size}) (drill ${p.socket_hole_drill}) (layers "*.Cu" "*.Mask") ${p.P102})
    (pad "27" thru_hole circle (at 0 10.16 ${p.r}) (size ${p.socket_hole_size} ${p.socket_hole_size}) (drill ${p.socket_hole_drill}) (layers "*.Cu" "*.Mask") ${p.P107})
    `
    const extra_pins_reversible = `
    (pad "28" thru_hole circle (at ${invert_pins ? '-' : ''}5.08 10.16 ${p.r}) (size ${p.socket_hole_size} ${p.socket_hole_size}) (drill ${p.socket_hole_drill}) (layers "*.Cu" "*.Mask") ${p.P101})
    (pad "29" thru_hole circle (at ${invert_pins ? '-' : ''}2.54 10.16 ${p.r}) (size ${p.socket_hole_size} ${p.socket_hole_size}) (drill ${p.socket_hole_drill}) (layers "*.Cu" "*.Mask") ${p.P102})
    `
